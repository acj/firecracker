...
mod tests {
    ...
    fn test_system_configuration() {
        ...
        let config_err = configure_system(&gm, GuestAddress(0), 0, &None, 1, BootProtocol::LinuxBoot);
        assert_eq!(
            config_err.unwrap_err(),
            super::ConfigurationError::MpTableSetup(mptable::MptableError::NotEnoughMemory)
        );

        configure_system(&gm, GuestAddress(0), 0, &None, no_vcpus, BootProtocol::PvhBoot).unwrap();

        // Now assigning some memory that falls before the 32bit memory hole.
        let mem_size = 128 << 20;
        let gm = arch_mem(mem_size);
        configure_system(&gm, GuestAddress(0), 0, &None, no_vcpus, BootProtocol::LinuxBoot).unwrap();
        configure_system(&gm, GuestAddress(0), 0, &None, no_vcpus, BootProtocol::PvhBoot).unwrap();
